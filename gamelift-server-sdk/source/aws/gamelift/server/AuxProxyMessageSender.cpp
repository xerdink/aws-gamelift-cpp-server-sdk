...
#include <aws/gamelift/internal/modelmapper/InstanceCertificateMapper.h>
...
GetInstanceCertificateOutcome AuxProxyMessageSender::GetInstanceCertificate()
{
    pbuffer::GetInstanceCertificate* event = new pbuffer::GetInstanceCertificate;
    return Call(event);
}
...
GenericOutcome AuxProxyMessageSender::EmitEvent(google::protobuf::MessageLite* message)
{
    ...
    std::function<void(sio::message::list const&)> ackFunction = std::bind([](sio::message::list const& msg, std::shared_ptr<std::promise<Aws::GameLift::GenericOutcome>> p)
    {
        ...
        if (!success)
        {
            //Let's parse the response and use the messages
            pbuffer::GameLiftResponse gameliftResponse;
            if (google::protobuf::util::JsonStringToMessage(msg[1]->get_string(), &gameliftResponse).ok())
            {
                GameLiftError gameLiftError = From(gameliftResponse);
                p->set_value(GenericOutcome(gameLiftError));
                return;
            }
...
DescribePlayerSessionsOutcome AuxProxyMessageSender::Call(pbuffer::DescribePlayerSessionsRequest* message)
{
    ...
    std::function<void(sio::message::list const&)> ackFunction = std::bind([](sio::message::list const& msg, std::shared_ptr<std::promise<DescribePlayerSessionsOutcome>> p)
    {
        ...
        if (!success)
        {
            //Parsing the error messages sent from the service.
            pbuffer::GameLiftResponse gameliftResponse;
            if (google::protobuf::util::JsonStringToMessage(msg[1]->get_string(), &gameliftResponse).ok())
            {
                GameLiftError gameLiftError = From(gameliftResponse);
                p->set_value(DescribePlayerSessionsOutcome(gameLiftError));
                return;
            }
        }
...
StartMatchBackfillOutcome AuxProxyMessageSender::Call(pbuffer::BackfillMatchmakingRequest* message)
{
    ...
    std::function<void(sio::message::list const&)> ackFunction = std::bind([](sio::message::list const& msg, std::shared_ptr<std::promise<StartMatchBackfillOutcome>> p)
    {
        ...
        if (!success)
        {
            ...
            {
                GameLiftError gameLiftError = From(gameliftResponse);
                p->set_value(StartMatchBackfillOutcome(gameLiftError));
                return;
            }
        }
...
GetInstanceCertificateOutcome AuxProxyMessageSender::Call(pbuffer::GetInstanceCertificate* message)
{
    std::shared_ptr<std::promise<GetInstanceCertificateOutcome>> promise = std::make_shared<std::promise<GetInstanceCertificateOutcome>>();

    // We bind a lambda to handle the ACK from AuxProxy. We have our lambda take a promise so we can resolve it later synchronously.
    std::function<void(sio::message::list const&)> ackFunction = std::bind([](sio::message::list const& msg, std::shared_ptr<std::promise<GetInstanceCertificateOutcome>> p)
    {
        if (msg.size() <= 1)
        {
            p->set_value(GetInstanceCertificateOutcome(GameLiftError(GAMELIFT_ERROR_TYPE::SERVICE_CALL_FAILED)));
            return;
        }
        bool success = msg[0]->get_bool();
        if (!success)
        ...
        pbuffer::GetInstanceCertificateResponse getInstanceCertificateResponse;
        if (!google::protobuf::util::JsonStringToMessage(msg[1]->get_string(), &getInstanceCertificateResponse).ok())
        {
            p->set_value(GetInstanceCertificateOutcome(GameLiftError(GAMELIFT_ERROR_TYPE::SERVICE_CALL_FAILED)));
            return;
        }

        GetInstanceCertificateResult result;
        InstanceCertificateMapper::ParseFromBufferedGetInstanceCertificateResponse(getInstanceCertificateResponse, result);
        p->set_value(GetInstanceCertificateOutcome(result));
        return;
    }, std::placeholders::_1, promise);
...
GameLiftError AuxProxyMessageSender::From(pbuffer::GameLiftResponse& gameliftResponse) {
    ...
    if (gameliftResponse.status() == pbuffer::GameLiftResponseStatus::ERROR_400) {
        ...
        gameLiftError = GameLiftError(GAMELIFT_ERROR_TYPE::SERVICE_CALL_FAILED);
    }
